...
import hasBigInt from 'has-bigints'

const hasSymbolsValue = (hasSymbols as any)()
const hasProtoValue = (hasProto as any)()
const hasBigIntValue = (hasBigInt as any)()
...
test('stringify()', function (t) {
  ...
  t.test('stringifies symbols', { skip: !hasSymbolsValue }, function (st) {
  ...
  t.test('stringifies bigints', { skip: !hasBigIntValue }, function (st) {
    const three = BigInt(3)
    const encodeWithN = function (value: any, defaultEncoder: any, charset: any) {
  ...
  t.test('throws when `commaRoundTrip` is not a boolean', function (st) {
    st['throws'](
      function () { qs.stringify({}, { commaRoundTrip: 'not a boolean' as any }) },
      TypeError,
  ...
  t.test('throws when `encodeDotInKeys` is not a boolean', function (st) {
    st['throws'](
      function () { qs.stringify({ a: [], b: 'zz' }, { encodeDotInKeys: 'foobar' as any }) },
      TypeError
    )

    st['throws'](
      function () { qs.stringify({ a: [], b: 'zz' }, { encodeDotInKeys: 0 as any }) },
      TypeError
    )

    st['throws'](
      function () { qs.stringify({ a: [], b: 'zz' }, { encodeDotInKeys: NaN as any }) },
      TypeError
    )

    st['throws'](
      function () { qs.stringify({ a: [], b: 'zz' }, { encodeDotInKeys: null as any }) },
      TypeError
    )
  ...
  t.test('should throw when allowEmptyArrays is not of type boolean', function (st) {
    st['throws'](
      function () { qs.stringify({ a: [], b: 'zz' }, { allowEmptyArrays: 'foobar' as any }) },
      TypeError
    )

    st['throws'](
      function () { qs.stringify({ a: [], b: 'zz' }, { allowEmptyArrays: 0 as any }) },
      TypeError
    )

    st['throws'](
      function () { qs.stringify({ a: [], b: 'zz' }, { allowEmptyArrays: NaN as any }) },
      TypeError
    )

    st['throws'](
      function () { qs.stringify({ a: [], b: 'zz' }, { allowEmptyArrays: null as any }) },
      TypeError
    )
  ...
  t.test('stringifies a null object', { skip: !hasProtoValue }, function (st) {
  ...
  t.test('stringifies an object with a null object as a child', { skip: !hasProtoValue }, function (st) {
  ...
  t.test('skips properties that are part of the object prototype', function (st) {
    st.intercept(Object.prototype as any, 'crash', { value: 'test' })
  ...
  t.test('stringifies buffer values', function (st) {
    st.equal(qs.stringify({ a: SaferBuffer.Buffer.from('test') }), 'a=test')
    st.equal(qs.stringify({ a: { b: SaferBuffer.Buffer.from('test') } }), 'a%5Bb%5D=test')
    st.end()
  })
  ...
  t.test('selects properties when filter=array', function (st) {
    ...
    st.equal(
      qs.stringify(
        { a: { b: [1, 2, 3, 4], c: 'd' }, c: 'f' },
        { filter: ['a', 'b', 0, 2] as any, arrayFormat: 'indices' }
      ),
    ...
    st.equal(
      qs.stringify(
        { a: { b: [1, 2, 3, 4], c: 'd' }, c: 'f' },
        { filter: ['a', 'b', 0, 2] as any, arrayFormat: 'brackets' }
      ),
  ...
  t.test('supports custom representations when filter=function', function (st) {
    ...
    const filterFunc = function (prefix: any, value: any) {
      calls += 1
      if (calls === 1) {
  ...
  t.test('can sort the keys', function (st) {
    const sort = function (a: string, b: string) {
      return a.localeCompare(b)
    }
    st.equal(qs.stringify({ a: 'c', z: 'y', b: 'f' }, { sort }), 'a=c&b=f&z=y')
    st.equal(qs.stringify({ a: 'c', z: { j: 'a', i: 'b' }, b: 'f' }, { sort }), 'a=c&b=f&z%5Bi%5D=b&z%5Bj%5D=a')
    st.end()
  })
  ...
  t.test('throws error with wrong encoder', function (st) {
    st['throws'](function () {
      qs.stringify({}, { encoder: 'string' as any })
    }, new TypeError('Encoder has to be a function.'))
    st.end()
  })

  t.test('can use custom encoder for a buffer object', { skip: typeof Buffer === 'undefined' }, function (st) {
    st.equal(qs.stringify({ a: SaferBuffer.Buffer.from([1]) }, {
    ...
    st.equal(qs.stringify({ a: SaferBuffer.Buffer.from('a b') }, {
      encoder: function (buffer) {
        return buffer
      },
    }), 'a=a b')
  ...
  t.test('serializeDate option', function (st) {
    ...
    st.equal(
      qs.stringify(
        { a: specificDate },
        { serializeDate: function (d: Date) { return String(d.getTime() * 7) } }
      ),
      'a=42',
    ...
    st.equal(
      qs.stringify(
        { a: [date] },
        {
          serializeDate: function (d: Date) { return String(d.getTime()) },
          arrayFormat: 'comma',
        }
      ),
      'a=' + date.getTime(),
  ...
  t.test('RFC 1738 serialization', function (st) {
    ...
    st.equal(qs.stringify({ 'a b': SaferBuffer.Buffer.from('a b') }, { format: qs.formats.RFC1738 }), 'a+b=a+b')
  ...
  t.test('RFC 3986 spaces serialization', function (st) {
    ...
    st.equal(qs.stringify({ 'a b': SaferBuffer.Buffer.from('a b') }, { format: qs.formats.RFC3986 }), 'a%20b=a%20b')
  ...
  t.test('Backward compatibility to RFC 3986', function (st) {
    st.equal(qs.stringify({ a: 'b c' }), 'a=b%20c')
    st.equal(qs.stringify({ 'a b': SaferBuffer.Buffer.from('a b') }), 'a%20b=a%20b')
  ...
  t.test('Edge cases and unknown formats', function (st) {
    ['UFO1234', false, 1234, null, {}, []].forEach(function (format) {
      st['throws'](
        function () {
          qs.stringify({ a: 'b c' }, { format: format as any })
        },
  ...
  t.test('throws if an invalid charset is specified', function (st) {
    st['throws'](function () {
      qs.stringify({ a: 'b' }, { charset: 'foobar' as any })
  ...
  t.test('strictNullHandling works with custom filter', function (st) {
    const filter = function (prefix: any, value: any) {
      return value
    }
  ...
  t.test('strictNullHandling works with null serializeDate', function (st) {
    const serializeDate = function (): any {
      return null
    }
    const options: any = { strictNullHandling: true, serializeDate }
    const date = new Date()
  ...
  t.test('allows for encoding keys and values differently', function (st) {
    const encoder = function (str: any, defaultEncoder: any, charset: any, type: any) {
      if (type === 'key') {
    ...
    st.deepEqual(qs.stringify({ KeY: 'vAlUe' }, { encoder }), 'key=VALUE')
    st.end()
  })
...
test('stringifies empty keys', function (t) {
  ...
  t.test('stringifies non-string keys', function (st) {
    ...
    const actual = qs.stringify({ a: 'b', false: {}, 1e+22: 'c', d: 'e' }, {
