...
import type { Configuration, AppenderFunction } from './types/core'

const debug = debugFactory('log4js:categories')
...
const categories = new Map<string, CategoryConfig>()
 ...
 * @return void
 */
function inheritFromParent (config: Configuration, category: Configuration['categories'][string], categoryName: string): void {
  ...
  if (!parentCategory) {
    ...
    parentCategory = { inherit: true, appenders: [], level: 'INFO' }
  }
  ...
  parentCategory.appenders.forEach((ap: string) => {
 ...
 * @param  config
 */
function addCategoryInheritance (config: Configuration): void {
...
configuration.addListener((config) => {
  ...
  categoryNames.forEach((name) => {
    ...
    configuration.throwExceptionIf(
      ...
      ` valid levels are ${Level.levels.map(l => l.levelStr).join(', ')})`
    )
  })
...
const setup = (config?: Configuration): void => {
  ...
  categoryNames.forEach((name) => {
    ...
    category.appenders.forEach((appender: string) => {
      const appenderFunction = appenders.get(appender)
      if (appenderFunction) {
        categoryAppenders.push(appenderFunction)
      }
...
const configForCategory = (category: string): CategoryConfig => {
  ...
  let sourceCategoryConfig: CategoryConfig
...
const getLevelForCategory = (category: string): Level => configForCategory(category).level
const setLevelForCategory = (category: string, level: Level): void => {
...
const setEnableCallStackForCategory = (category: string, useCallStack: boolean): void => {
