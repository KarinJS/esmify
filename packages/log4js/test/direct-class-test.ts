...
import { Logger, LevelClass, LoggingEvent, createLevel } from '../src/index'
...
test('LoggingEvent direct test', (t) => {
  // 基本创建
  const event = new LoggingEvent('test-category', (LevelClass as any).INFO, ['test message'])

  t.ok(event, 'LoggingEvent should be created')
  t.equal(event.categoryName, 'test-category', 'should have correct category')
  t.equal(event.level, (LevelClass as any).INFO, 'should have correct level')
  ...
  const event2 = new LoggingEvent('test', (LevelClass as any).ERROR, ['error'], context)
  ...
  const event3 = new LoggingEvent('test', (LevelClass as any).WARN, ['warning'], undefined, location)
  ...
  // 无效location测试
  t.throws(() => {
    new LoggingEvent('test', (LevelClass as any).INFO, ['message'], undefined, [] as any)
...
test('Level class direct test', (t) => {
  // 测试Level构造
  const customLevel = createLevel(15000, 'CUSTOM', 'purple')
  ...
  // 测试比较方法
  const LevelAny = LevelClass as any
  ...
  // 测试getLevel
  t.equal(LevelClass.getLevel('DEBUG'), debug, 'getLevel should work')
  t.equal(LevelClass.getLevel(debug), debug, 'getLevel should work with Level')
  t.equal(LevelClass.getLevel('invalid', info), info, 'getLevel should return default for invalid')

  // 测试addLevels
  t.doesNotThrow(() => {
    LevelClass.addLevels({
      TEST: { value: 25000, colour: 'orange' },
    })
  }, 'addLevels should work')

  t.equal((LevelClass as any).TEST.levelStr, 'TEST', 'addLevels should add new level')
