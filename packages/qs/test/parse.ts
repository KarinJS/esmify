...
import hasOverrideMistake from 'has-override-mistake'
import SaferBuffer from 'safer-buffer'
import v from 'es-value-fixtures'
import inspect from 'object-inspect'
import { emptyTestCases } from './empty-keys-cases'
import hasProto from 'has-proto'

import * as qs from '../src/index'
import * as utils from '../src/utils'

const hasPropertyDescriptorsValue = hasPropertyDescriptors()
const hasOverrideMistakeValue = hasOverrideMistake()
const hasProtoValue = hasProto()
...
test('parse()', function (t) {
  ...
  t.test('throws when decodeDotInKeys is not of type boolean', function (st) {
    st['throws'](
      function () { qs.parse('foo[]&bar=baz', { decodeDotInKeys: 'foobar' as any }) },
      TypeError
    )

    st['throws'](
      function () { qs.parse('foo[]&bar=baz', { decodeDotInKeys: 0 as any }) },
      TypeError
    )
    st['throws'](
      function () { qs.parse('foo[]&bar=baz', { decodeDotInKeys: NaN as any }) },
      TypeError
    )

    st['throws'](
      function () { qs.parse('foo[]&bar=baz', { decodeDotInKeys: null as any }) },
      TypeError
    )
  ...
  t.test('throws when allowEmptyArrays is not of type boolean', function (st) {
    st['throws'](
      function () { qs.parse('foo[]&bar=baz', { allowEmptyArrays: 'foobar' as any }) },
      TypeError
    )

    st['throws'](
      function () { qs.parse('foo[]&bar=baz', { allowEmptyArrays: 0 as any }) },
      TypeError
    )
    st['throws'](
      function () { qs.parse('foo[]&bar=baz', { allowEmptyArrays: NaN as any }) },
      TypeError
    )

    st['throws'](
      function () { qs.parse('foo[]&bar=baz', { allowEmptyArrays: null as any }) },
      TypeError
    )
  ...
  t.test('uses original key when depth = false', function (st) {
    st.deepEqual(qs.parse('a[0]=b&a[1]=c', { depth: false as any }), { 'a[0]': 'b', 'a[1]': 'c' })
    st.deepEqual(qs.parse('a[0][0]=b&a[0][1]=c&a[1]=d&e=2', { depth: false as any }), { 'a[0][0]': 'b', 'a[0][1]': 'c', 'a[1]': 'd', e: '2' })
    st.end()
  })
  ...
  t.test('allows empty values', function (st) {
    ...
    st.deepEqual(qs.parse(null as any), {})
    st.deepEqual(qs.parse(undefined as any), {})
    st.end()
  })
  ...
  t.test('cannot access Object prototype', function (st) {
    ...
    st.equal(typeof (Object.prototype as any).bad, 'undefined')
    st.end()
  })
  ...
  t.test('parses semi-parsed strings', function (st) {
    st.deepEqual(qs.parse({ 'a[b]': 'c' } as any), { a: { b: 'c' } })
    st.deepEqual(qs.parse({ 'a[b]': 'c', 'a[d]': 'e' } as any), { a: { b: 'c', d: 'e' } })
    st.end()
  })

  t.test('parses buffers correctly', function (st) {
    const b = SaferBuffer.Buffer.from('test')
    st.deepEqual(qs.parse({ a: b } as any), { a: b })
    st.end()
  })
  ...
  t.test('does not throw when a native prototype has an enumerable property', function (st) {
    st.intercept(Object.prototype as any, 'crash', { value: '' })
    st.intercept(Array.prototype as any, 'crash', { value: '' })
  ...
  t.test('does not use non-splittable objects as delimiters', function (st) {
    st.deepEqual(qs.parse('a=b&c=d', { delimiter: true as any }), { a: 'b', c: 'd' })
    st.end()
  })
  ...
  t.test('parses an object', function (st) {
    ...
    const result = qs.parse(input as any)
  ...
  t.test('parses values with comma as array divider', function (st) {
    st.deepEqual(qs.parse({ foo: 'bar,tee' } as any, { comma: false }), { foo: 'bar,tee' })
    st.deepEqual(qs.parse({ foo: 'bar,tee' } as any, { comma: true }), { foo: ['bar', 'tee'] })
    st.end()
  })

  t.test('use number decoder, parses string that has one number with comma option enabled', function (st) {
    const decoder = function (str: any, defaultDecoder: any, charset: any, type: any) {
    ...
    st.deepEqual(qs.parse('foo=1', { comma: true, decoder }), { foo: 1 })
    st.deepEqual(qs.parse('foo=0', { comma: true, decoder }), { foo: 0 })
  ...
  t.test('parses an object in dot notation', function (st) {
    ...
    const result = qs.parse(input as any, { allowDots: true })
  ...
  t.test('does not blow up when Buffer global is missing', function (st) {
    const restore = mockProperty(global, 'Buffer', { delete: true })
  ...
  t.test('does not crash when parsing circular references', function (st) {
    ...
    st.doesNotThrow(function () {
      parsed = qs.parse({ 'foo[bar]': 'baz', 'foo[baz]': a } as any)
    })
  ...
  t.test('parses null objects correctly', { skip: !hasProtoValue }, function (st) {
    const a = { __proto__: null, b: 'c' }

    st.deepEqual(qs.parse(a as any), { b: 'c' })
    const result = qs.parse({ a } as any)
  ...
  t.test('parses dates correctly', function (st) {
    const now = new Date()
    st.deepEqual(qs.parse({ a: now } as any), { a: now })
    st.end()
  })

  t.test('parses regular expressions correctly', function (st) {
    const re = /^test$/
    st.deepEqual(qs.parse({ a: re } as any), { a: re })
    st.end()
  })
  ...
  t.test('does not crash when the global Object prototype is frozen', { skip: !hasPropertyDescriptorsValue || !hasOverrideMistakeValue }, function (st) {
    ...
    st.teardown(mockProperty(Object.prototype as any, 'frozenProp', { value: 'foo', nonWritable: true, nonEnumerable: true }))
  ...
  t.test('can return null objects', { skip: !hasProtoValue }, function (st) {
    const expected = {
      __proto__: null,
      a: {
        __proto__: null,
    ...
    st.deepEqual(qs.parse(null as any, { plainObjects: true }), { __proto__: null })
    const expectedArray = {
      __proto__: null,
      a: {
        __proto__: null,
  ...
  t.test('can parse with custom encoding', function (st) {
    st.deepEqual(qs.parse('%8c%a7=%91%e5%8d%e3%95%7b', {
      decoder: function (str) {
        ...
        while (parts) {
          result.push(Number.parseInt(parts[1], 16))
          parts = reg.exec(str)
        }
        return String(iconv.decode(SaferBuffer.Buffer.from(result), 'shift_jis'))
      },
    }), { 県: '大阪府' })
    st.end()
  })
  ...
  t.test('throws error with wrong decoder', function (st) {
    st['throws'](function () {
      qs.parse({}, { decoder: 'string' as any })
  ...
  t.test('throws if an invalid charset is specified', function (st) {
    st['throws'](function () {
      qs.parse('a=b', { charset: 'foobar' as any })
  ...
  t.test('allows for decoding keys and values differently', function (st) {
    ...
    st.deepEqual(qs.parse('KeY=vAlUe', { decoder }), { key: 'VALUE' })
    st.end()
  })
  ...
  t.test('parameter limit tests', function (st) {
    ...
    st.test('throws error when throwOnLimitExceeded is present but not boolean', function (sst) {
      sst['throws'](
        function () {
          qs.parse('a=1&b=2&c=3&d=4&e=5&f=6', { parameterLimit: 3, throwOnLimitExceeded: 'true' as any })
        },
  ...
  t.test('array limit tests', function (st) {
    ...
    st.test('throws error when throwOnLimitExceeded is present but not boolean for array limit', function (sst) {
      sst['throws'](
        function () {
          qs.parse('a[]=1&a[]=2&a[]=3&a[]=4', { arrayLimit: 3, throwOnLimitExceeded: 'true' as any })
        },
        new TypeError('`throwOnLimitExceeded` option must be a boolean'),
...
test('`duplicates` option', function (t) {
  (v.nonStrings as any[]).concat('not a valid option').forEach(function (invalidOption: any) {
    if (typeof invalidOption !== 'undefined') {
      t['throws'](
        function () { qs.parse('', { duplicates: invalidOption as any }) },
        TypeError,
...
test('qs strictDepth option - non-throw cases', function (t) {
  ...
  t.test('parses successfully when depth is within the limit with strictDepth: true', function (st) {
    st.doesNotThrow(
      function () {
        const result = qs.parse('a[b]=c', { depth: 1, strictDepth: true })
  ...
  t.test('does not throw an exception when depth exceeds the limit with strictDepth: false', function (st) {
    st.doesNotThrow(
      function () {
        const result = qs.parse('a[b][c][d][e][f][g][h][i]=j', { depth: 1 })
  ...
  t.test('parses successfully when depth is within the limit with strictDepth: false', function (st) {
    st.doesNotThrow(
      function () {
        const result = qs.parse('a[b]=c', { depth: 1 })
        st.deepEqual(result, { a: { b: 'c' } }, 'parses correctly')
      }
    )
    st.end()
  ...
  t.test('does not throw when depth is exactly at the limit with strictDepth: true', function (st) {
    st.doesNotThrow(
      function () {
        const result = qs.parse('a[b][c]=d', { depth: 2, strictDepth: true })
