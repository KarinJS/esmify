...
export class CronFieldCollection {
  ...
  constructor({ second, minute, hour, dayOfMonth, month, dayOfWeek }: CronFields) {
    ...
    if (month.values.length === 1 && !dayOfMonth.hasLastChar) {
      const dayValue = String(dayOfMonth.values[0]);
      // Skip validation for hash syntax (H character)
      if (!dayValue.includes('H')) {
        const dayNum = parseInt(dayValue, 10);
        if (!(dayNum <= CronMonth.daysInMonth[month.values[0] - 1])) {
